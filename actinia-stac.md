...
You can run actinia-stac-plugin in multiple ways (see README.md). If actinia-stac-plugin is installed as actinia-core plugin, endpoints for module self-description are added. These modules are a collection of common GRASS GIS modules (grass-module) and of specific actinia-modules. While GRASS GIS is able to tell input/output parameters for a certain GRASS GIS module via `--interface-description` which can easily be converted to JSON, actinia-modules are more complex.
...
* A client with autogenerated requests talks to the API directly and needs a consistent way to address predefined processes.
Therefore actinia-stac-plugin is able to store process-chain templates and the user only needs to enter certain values. A template might look like this:
...
```
This way, the user only needs to set values for the defined variables. The user doesn't even need to know the whole template. actinia-stac-plugin will translate it into one actinia-module, exploiting only the values necessary for input:
...
For the user this looks like one module in the process-chain. In further development, it will be shrunk even more to not exploit the same variable twice. This might then look like this:
...
Both are special modules from actinia-core. Safety mechanisms are already implemented and optimized, e.g. clean up of download cache from node / feature to block download if it threatens health of node...
...
#### The self-description tries to comply the [openEO API](https://api.openeo.org/#section/Processes) where applicable.
...
These are neiher fully grass-modules nor fully actinia-modules. They need to be added to GRASS GIS with g.extension (originally for ace usage) but are extended by actinia-stac-plugin. The GRASS interface description does not describe import_descr and export as seen in API docs, so this is extended. It is possible to extend all GRASS modules by importer/exporter attributes if applicable (e.g. output of v.buffer can directly be exported with "export" attribute inside module)
